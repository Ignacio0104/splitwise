...
export default function TabButtons({ size, buttonName, bottomProps }: tabButtonsProps) {
  const styles = useStyles();

  const buttonColor = bottomProps.accessibilityState?.selected ? Theme.greenHiglight : Theme.grayFont;
  ...
  const renderIcon = (): React.JSX.Element => {
    switch (buttonName) {
      ...
      case 'friends':
        return <FontAwesome5 name="user-friends" size={size} color={buttonColor} />;
      default:
  ...
  return (
    <TouchableOpacity
      ...
      style={buttonName === 'creation' ? styles.creationStyle : styles.iconContainer}
    >
      {renderIcon()}
    </TouchableOpacity>
  );
